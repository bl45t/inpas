...
class ControllerCatalogManufacturer extends Controller {
    ...
    if ($this->config->get('config_editor_default')) {
		...
		$data['entry_educational_program'] = $this->language->get('entry_educational_program');
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['manufacturer_description'] as $language_id => $value) {
			...
			if (utf8_strlen(trim($value['phone'])) == 0) {
				$this->error['phone'][$language_id] = $this->language->get('error_phone');
			}

			if ((utf8_strlen($value['phone']) > 96) || !preg_match($this->config->get('config_mail_regexp'), $value['email'])) {
